...
class AwsInvokeLocal {
  ...
  async pullDockerImage() {
    const baseImageName = this.getDockerBaseImageName();

    dockerProgress.notice(`Downloading base Docker image ${baseImageName}`);
    // --disable-content-trust=false has been removed until AWS ECR Public supports Docker Content Trust
    // See https://github.com/aws/containers-roadmap/issues/43#issuecomment-1447514149
    return spawnExt('docker', ['pull', baseImageName]);
  }
  ...
  getDockerBaseImageName() {
    // TODO need to map this into the https://gallery.ecr.aws/lambda/ namespace - e.g. pull out the runtime, convert it
    // Implement https://github.com/aws/aws-sam-cli/blob/129ac47be232a93779699f4372938ad3198c3ff4/samcli/local/docker/lambda_image.py#LL63C6-L63C6
    // in Javascript
    return 'public.ecr.aws/lambda/provided:al2';
    // return `lambci/lambda:${this.getRuntime()}`; // EOL many years ago
    // return `amazon/aws-sam-cli-emulation-image-${this.getRuntime()}:latest`; // Outdated, only publishes x86_64 images
    // return `public.ecr.aws/sam/emulation-${this.getRuntime()}:latest`; // Outdated - URLs changed again in 2023
  }
  ...
  async buildDockerImage(layerPaths) {
    ...
    let dockerFileContent = `FROM ${this.getDockerBaseImageName()}`;
    ...
    dockerProgress.notice(`Building Docker image ${imageName}`);
    try {
  ...
  async invokeLocalDocker() {
    ...
    const results = await Promise.all([
      this.checkDockerImage(this.getDockerBaseImageName()).then((exists) => {
    ...
    // TODO - for "provided" we need to do things differently
    ...
    const dockerArgs = [
      ...
      // TODO needs to change for provided
      [imageName, handler]
